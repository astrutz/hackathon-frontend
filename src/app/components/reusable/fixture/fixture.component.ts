...
export class FixtureComponent {
  ...
  get player1(): string {
    if (this.is1v1A) {
      return this.dataService.getPlayerById(this.game.team1Players?.[0])?.name ?? '[Deaktivierter Nutzer]';
    }
  ...
  get player2(): string {
    if (this.is1v1B) {
      return this.dataService.getPlayerById(this.game.team2Players?.[0])?.name ?? '[Deaktivierter Nutzer]';
    }
